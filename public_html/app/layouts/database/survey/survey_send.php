...
function survey_send($print){
    ...
    if (isset($_POST['submitsurvey'])) {
        ...
        $data = array(
            // Column => Value
            'userid' => safeinteger($userid),
            'displayid' => safestring($_POST['displayid']),
            'clientname' => safestring($_POST['your-name']),
            'clientemail' => safeemail($_POST['your-email']),
            'region' => safeinteger($_POST['region']),
            'clientaccount' => safestring($_POST['your-company']),
            'emailtitle' => safestring($_POST['emailtitle']),
            'emaildesc' => safestring($_POST['emaildesc'])
        );
        $format = array(
            // Format
            '%d',
        ...
        // ------ QUERY ------
        $survey_send = $wpdb->insert($table, $data, $format);
        ...
        if ($_POST['emailtitle']) {
            $clienttitle = safestring($_POST['emailtitle']);
        } else {
            $clienttitle = 'Service Feedback Survey';
        }
        if ($_POST['emaildesc']) {
            $clientdescription = '<br><br><span>' . safestring(stripslashes($_POST['emaildesc'])) . '</span>';
        } else {
            $clientdescription = '';
        }
