...
function permissions_regions_all($print){
    ...
    if (doif_coopereditoronly_query()) {
        global $permissions_regions_all;
        $permissions_regions_all = $wpdb->get_results("SELECT * FROM `$table` WHERE userid = $user_id");
    }

    // ------ BUG CHECKING ------
    if ($print == true) {
        print_r($permissions_regions_all);
    }
