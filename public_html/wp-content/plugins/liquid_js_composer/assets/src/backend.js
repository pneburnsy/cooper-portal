...
window.vc || (window.vc = {}),
	...
	function ($) {
		...
		}), window.vc.PagesTemplateWindowUIPanelBackendEditor = vc.TemplateWindowUIPanelBackendEditor.extend({
			initialize: function () {
				window.vc.PagesTemplateWindowUIPanelBackendEditor.__super__.initialize.call(this);
			},
	...
	function ($) {
		...
		window.vc.element_start_index = 0, window.vc.AddElementUIPanelBackendEditor = vc.PanelView.vcExtendUI(vc.HelperPanelViewHeaderFooter).extend({
			...
			render: function (model, prepend) {
				...
				this.$el.find('[data-vc-ui-element="panel-tabs-controls"]').vcTabsLine("moveTabs");
				vc.is_mobile || $(this.searchSelector).focus();
			...
			cloneTabControls: function() {
				this.$el.find('[data-vc-ui-element="panel-tab-control"]').each((i, btn) => {
					...
					$toggleBtn.clone(false).insertBefore($firstButton).removeAttr('class data-vc-ui-element');
				})
			},
			buildFiltering: function () {
	...
	function ($) {
		...
		window.vc.EditElementPanelView = vc.PanelView.vcExtendUI(vc.HelperAjax).vcExtendUI(vc.ExtendPresets).vcExtendUI(vc.ExtendTemplates).vcExtendUI(vc.HelperPrompts).extend({
			...
			initParams: function () {
				...
				}), $content.attr("data-vc-param-initialized", !0));
				this.requiredParamsInitialized || _.isUndefined(vc.required_params_to_init) || ($('[data-vc-ui-element="panel-shortcode-param"]', this.content()).each(function () {
					var $field, param;
			...
			setTitle: function () {
				this.$el.find(this.titleSelector).text(vc.getMapped(this.model.get("shortcode")).name + " " + window.i18nLocale.settings);
				return this
			},
	...
	function ($) {
		...
		var GoogleFonts = Backbone.View.extend({
			...
			VC_ParamGroup = Backbone.View.extend({
				...
				afterAdd: function ($newEl, action) {
					var fn;
					this.addAllowed() || (this.$ul.find("> .wpb_vc_row > .vc_param_group-controls > .vc_row_edit_clone_delete > .vc_control.column_clone").hide(), this.$ul.find("> .vc_param_group-add_content").hide());
					this.options.sortable || this.$ul.find("> .wpb_vc_row > .vc_param_group-controls > .vc_control.column_move").hide();
					this.options.deletable || this.$ul.find("> .wpb_vc_row > .vc_param_group-controls > .vc_row_edit_clone_delete > .vc_control.column_delete").hide();
					this.options.collapsible || this.$ul.find("> .wpb_vc_row > .vc_param_group-controls > .vc_row_edit_clone_delete > .vc_control.column_toggle").hide();
					void 0 !== this.options.param.callbacks && void 0 !== this.options.param.callbacks.after_add && "function" == typeof (fn = window[this.options.param.callbacks.after_add]) && fn($newEl, action)
				},
				afterDelete: function () {
					var fn;
					this.addAllowed() && (this.$ul.find("> .wpb_vc_row > .vc_param_group-controls > .vc_row_edit_clone_delete > .vc_control.column_clone").show(), this.$ul.find("> .vc_param_group-add_content").show());
					void 0 !== this.options.param.callbacks && void 0 !== this.options.param.callbacks.after_delete && "function" == typeof (fn = window[this.options.param.callbacks.after_delete]) && fn()
				}
			}),
		...
		vc.atts.attach_images = {
			...
			updateImages: function ($thumbnails, thumbnails_html) {
				$thumbnails.html(thumbnails_html), thumbnails_html.length ? $thumbnails.removeClass("image-exists").siblings('.no_image_image').addClass("image-exists") : $thumbnails.addClass("image-exists").siblings('.no_image_image').removeClass("image-exists")
			}
		};
		vc.atts.href = {
			parse: function (param) {
		...
		vc.atts.attach_image = {
			...
			updateImage: function ($thumbnail, image_src, image_exists) {
				$thumbnail.length && (void 0 === image_exists && (image_exists = !1), image_exists || !_.isEmpty(image_src) ? ($thumbnail.attr("src", image_src), _.isEmpty(image_src) ? ($thumbnail.hide(), $thumbnail.siblings('.no_image_image').removeClass("image-exists").siblings('.no_image_image').addClass("image-exists")) : ($thumbnail.show(), $thumbnail.siblings('.no_image_image').addClass("image-exists").siblings('.no_image_image').addClass("image-exists"))) : $thumbnail.attr("src", "").hide().siblings('.no_image_image').removeClass("image-exists").siblings('.no_image_image').removeClass("image-exists"))
			}
		};
	...
	function ($) {
		...
		vc.saved_custom_css = !1, vc.createPreLoader = function () {
			...
			events: {
				...
				"click #vc_templates-more-pages": "openPagesTemplatesWindow",
			...
			render: function () {
				...
				}), vc.pages_templates_panel_view = new vc.PagesTemplateWindowUIPanelBackendEditor({
			...
			openTemplatesWindow: function (e) {
				e && e.preventDefault && e.preventDefault();
				$(e.currentTarget).is("#vc_templates-more-layouts") && vc.templates_panel_view.once("show", function () {
	...
	function ($) {
		...
		window.VcRowView = vc.shortcode_view.extend({
			...
			buildDesignHelpers: function () {
				var css, image, color, rowId, matches;
				css = this.model.getParam("css"),
				(matches = css.match(/background\-image:\s*url\(([^\)]+)\)/)) && !_.isUndefined(matches[1]) && (image = matches[1]);
				(matches = css.match(/background\-color:\s*([^\s\;]+)\b/)) && !_.isUndefined(matches[1]) && (color = matches[1]);
				(matches = css.match(/background:\s*([^\s]+)\b\s*url\(([^\)]+)\)/)) && !_.isUndefined(matches[1]) && (color = matches[1], image = matches[2]);
				rowId = this.model.getParam("el_id");
				_.isEmpty(rowId) || this.$el.find(this.designHelpersSelector).siblings('.vc_row-hash-id').text(`#${rowId}`);
				image && this.$el.find(this.designHelpersSelector).siblings('.vc_row_image').removeClass('vc_empty').css('backgroundImage', `url(${image})`) || this.$el.find(this.designHelpersSelector).siblings('.vc_row_image').addClass('vc_empty');
				color && this.$el.find(this.designHelpersSelector).siblings('.vc_row_color').removeClass('vc_empty').css('backgroundColor', `url(${color})`) || this.$el.find(this.designHelpersSelector).siblings('.vc_row_color').addClass('vc_empty');
			},
			addElement: function (e) {
			...
			initialize: function (options) {
				...
				if ( alignParam && alignParam !== '' ) {
					...
					params['responsive_align'] = newResponsiveAlignArr.join(' ');
					this.model.set('params', params, {silent: true});
					this.model.save();
			...
			setColumnClasses: function () {
				...
				this.current_column_width !== width && (current_css_class_width = this.convertSize(this.current_column_width), this.$el.attr("data-width", width).removeClass(current_css_class_width).addClass(css_class_width), this.current_column_width = width);
				offset.match(/vc_col\-sm\-\d+/) && this.$el.removeClass(css_class_width);
				_.isEmpty(offset) || this.$el.addClass(offset);
			...
			buildDesignHelpers: function () {
				var css, image, color, elId, matches;
				css = this.model.getParam("css");
				(matches = css.match(/background\-image:\s*url\(([^\)]+)\)/)) && !_.isUndefined(matches[1]) && (image = matches[1]);
				(matches = css.match(/background\-color:\s*([^\s\;]+)\b/)) && !_.isUndefined(matches[1]) && (color = matches[1]);
				(matches = css.match(/background:\s*([^\s]+)\b\s*url\(([^\)]+)\)/)) && !_.isUndefined(matches[1]) && (color = matches[1], image = matches[2]);
				elId = this.model.getParam("el_id");
				_.isEmpty(elId) || this.$el.find(this.designHelpersSelector).siblings('.vc_row-hash-id').text(`#${elId}`);
				image && this.$el.find(this.designHelpersSelector).siblings('.vc_row_image').removeClass('vc_empty').css('backgroundImage', `url(${image})`) || this.$el.find(this.designHelpersSelector).siblings('.vc_row_image').addClass('vc_empty');
				color && this.$el.find(this.designHelpersSelector).siblings('.vc_row_color').removeClass('vc_empty').css('backgroundColor', `url(${color})`) || this.$el.find(this.designHelpersSelector).siblings('.vc_row_color').addClass('vc_empty');
			},
			checkIsEmpty: function () {
