...
window.vc_iframe = {
	...
	function ($) {
		...
		window.vc_iframe.showNoContent = function (show) {
			...
			parent.vc.$page.addClass("vc-main-sortable-container").removeClass('container');
			...
			$main.sortable({
				...
				start: function (event, ui) {
					...
					"vc_section" === ui.item.data("tag") ? ($sectionSortables && $sectionSortables.sortable("destroy"), $rowSortable && $rowSortable.sortable("destroy"), $elementsSortable && $elementsSortable.sortable("destroy"), $main.sortable("option", "connectWith", !1)) : $main.sortable("option", "connectWith", ['[data-tag="vc_section"] > .vc_element-container']);
				...
				$elementsSortable = $(".vc_element-container:not(.vc_section)").sortable({
					...
					change: function (event, ui) {
						ui.placeholder.css('minHeight', 30);
						ui.placeholder.width(ui.item.parent().width())
					},
				...
				$rowSortable = $(".vc_row").sortable({
					forcePlaceholderSize: !0,
					tolerance: "pointer",
					items: "> .ld-container > .ld-row > [data-tag=vc_column], > .ld-container > .ld-row > [data-tag=vc_column_inner]",
					handle: ".vc_controls .element-vc_column .vc_element-move, .vc_controls .element-vc_column_inner .vc_element-move",
					start: function (event, ui) {
						window.vc_iframe.startSorting();
						var id = ui.item.data("modelId"),
							model = parent.vc.shortcodes.get(id),
							css_class = model.view.$el.attr('class').split(' ').filter(classname => classname.includes('vc_col-')),
							w = ui.item.width(),
							h = ui.item.height();
