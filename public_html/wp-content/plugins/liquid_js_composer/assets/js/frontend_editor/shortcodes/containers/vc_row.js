(function ( $ ) {
	...
	window.InlineShortcodeView_vc_row = window.InlineShortcodeView.extend( {
		...
		updated: function () {
			...
			if ( vc.frame_window.jQuery('body').hasClass('pp-enabled') ) {
				const $siteContent = vc.frame_window.jQuery('#lqd-site-content');
				const stackData = $siteContent.data('plugin_liquidStack')
				if ( stackData ) {
					this.$el.addClass('pp-section pp-table');
				}
			}
		},
		changed: function () {
		...
		reInitLiquidStack: function() {
			const $siteContent = vc.frame_window.jQuery('#lqd-site-content');
			const stackData = $siteContent.data('plugin_liquidStack')
			console.log(stackData);
			if ( stackData ) {
				// stackData.$pageNumber && stackData.$pageNumber.remove();
				// stackData.$ppNav && stackData.$ppNav.remove();
				// stackData.$prevNextButtons && stackData.$prevNextButtons.remove();
				// $siteContent.data('plugin_liquidStack', null);
				this.$el.addClass('pp-section pp-table');
				if ( ! this.$el.children('.pp-tableCell').length ) {
					this.$el.wrapInner('<div class="pp-tableCell" style="height:100%"><div class="lqd-stack-section-inner"></div></div>')
				};
				// $siteContent.liquidStack({
