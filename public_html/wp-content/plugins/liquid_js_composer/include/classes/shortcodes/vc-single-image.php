...
class WPBakeryShortCode_Vc_Single_Image extends WPBakeryShortCode {
	...
	public function singleParamHtmlHolder( $param, $value ) {
		...
		if ( 'attach_image' === $param['type'] && 'image' === $param_name ) {
			...
			$this->setSettings( 'logo', ( $img ? $img['thumbnail'] : '<span class="vc_general vc_element-icon no_image_image vc_element-icon' . ( ! empty( $element_icon ) ? ' ' . $element_icon : '' ) . ( $img && ! empty( $img['p_img_large'][0] ) ? ' image-exists' : '' ) . '"></span><img width="150" height="150" src="' . esc_url( vc_asset_url( 'vc/blank.gif' ) ) . '" class="attachment-thumbnail vc_general vc_element-icon"  data-name="' . $param_name . '" alt="" title="" style="display: none;" />' ) . '<a href="#" class="column_edit_trigger' . ( $img && ! empty( $img['p_img_large'][0] ) ? ' image-exists' : '' ) . '">' . esc_html__( 'Add image', 'js_composer' ) . '</a>' );
