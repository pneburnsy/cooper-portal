...
class WPBakeryShortCode_Vc_Section extends WPBakeryShortCodesContainer {
	...
	public function getColumnControls( $controls = 'full', $extended_css = '' ) {
		...
		$output = '<div class="vc_controls vc_controls-row controls_row">';
		$controls_end = '</div>';
		// Create columns
		$controls_move = ' <a class="vc_control column_move vc_column-move" href="#" title="' . esc_attr__( 'Drag row to reorder', 'js_composer' ) . '" data-vc-control="move"><i class="la la-arrows"></i></a>';
		$moveAccess = vc_user_access()->part( 'dragndrop' )->checkStateAny( true, null )->get();
		if ( ! $moveAccess ) {
			$controls_move = '';
		}
		$controls_add = ' <a class="vc_control column_add vc_column-add" href="#" title="' . esc_attr__( 'Add column', 'js_composer' ) . '" data-vc-control="add"><i class="la la-plus"></i></a>';
		$controls_delete = '<a class="vc_control column_delete vc_column-delete" href="#" title="' . esc_attr__( 'Delete this row', 'js_composer' ) . '" data-vc-control="delete"><i class="la la-times"></i></a>';
		$controls_edit = ' <a class="vc_control column_edit vc_column-edit" href="#" title="' . esc_attr__( 'Edit this row', 'js_composer' ) . '" data-vc-control="edit"><i class="la la-pen"></i></a>';
		$controls_clone = ' <a class="vc_control column_clone vc_column-clone" href="#" title="' . esc_attr__( 'Clone this row', 'js_composer' ) . '" data-vc-control="clone"><i class="la la-copy"></i></a>';
		$controls_color = ' <span class="vc_row_color vc_empty" title="' . esc_attr__( 'Row background color', 'js_composer' ) . '"></span>';
		$controls_img = ' <span class="vc_row_image vc_empty" title="' . esc_attr__( 'Row background image', 'js_composer' ) . '"></span>';
		$controls_hash_id = ' <span class="vc_row-hash-id"></span>';
		...
		if ( $allAccess ) {
			...
			$row_edit_clone_delete .= $controls_color;
			$row_edit_clone_delete .= $controls_img;
		} elseif ( $editAccess ) {
			$row_edit_clone_delete .= $controls_edit;
		}
		$row_edit_clone_delete .= $controls_hash_id;
