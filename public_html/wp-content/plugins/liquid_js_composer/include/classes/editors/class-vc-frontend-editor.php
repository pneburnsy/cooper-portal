...
class Vc_Frontend_Editor {
			...
			if ( isset( $shortcode['id'] ) && isset( $shortcode['string'] ) ) {
				if ( isset( $shortcode['tag'] ) ) {
					...
					if ( is_object( $shortcode_obj ) ) {
						...
						$output .= '<div class="vc_element" data-tag-name="' .  $shortcode_obj->settings( 'name' ) . '" data-shortcode-controls="' . esc_attr( wp_json_encode( $shortcode_obj->shortcodeClass()
	...
	public function toString( $shortcode, $content ) {
		...
		$output = sprintf( '<div class="vc_element" data-tag-name="%s" data-tag="%s" data-shortcode-controls="%s" data-model-id="%s">%s[%s %s]%s[/%s]%s</div>', esc_attr( $shortcode_obj->settings( 'name' ) ), esc_attr( $shortcode['tag'] ), esc_attr( wp_json_encode( $shortcode_obj->shortcodeClass()
