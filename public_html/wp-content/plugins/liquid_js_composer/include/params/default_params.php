...
function vc_checkbox_form_field( $settings, $value ) {
	...
	if ( ! empty( $values ) ) {
		foreach ( $values as $label => $v ) {
			...
			$output .= ' <label class="vc_checkbox-label"><input id="' . $settings['param_name'] . '-' . $v . '" value="' . $v . '" class="wpb_vc_param_value ' . $settings['param_name'] . ' ' . $settings['type'] . '" type="checkbox" name="' . $settings['param_name'] . '" ' . $checked . '><span>' . $label . '</span></label>';
		}
	}
...
function vc_posttypes_form_field( $settings, $value ) {
	...
	foreach ( $post_types as $post_type ) {
		...
		if ( 'attachment' !== $post_type ) {
			...
			$output .= '<label class="vc_checkbox-label"><input id="' . $settings['param_name'] . '-' . $post_type . '" value="' . $post_type . '" class="wpb_vc_param_value ' . $settings['param_name'] . ' ' . $settings['type'] . '" type="checkbox" name="' . $settings['param_name'] . '" ' . $checked . '><span>' . $post_type . '</span></label>';
		}
	}
...
function vc_taxonomies_form_field( $settings, $value ) {
	...
	foreach ( $post_types as $type ) {
		...
		foreach ( $taxonomies as $tax ) {
			...
			$output .= ' <label class="vc_checkbox-label" data-post-type="' . $type . '"><input id="' . $settings['param_name'] . '-' . $tax->name . '" value="' . $tax->name . '" data-post-type="' . $type . '" class="wpb_vc_param_value ' . $settings['param_name'] . ' ' . $settings['type'] . '" type="checkbox" name="' . $settings['param_name'] . '" ' . $checked . '><span>' . $tax->label . '</span></label>';
		}
	}
...
function vc_attach_images_form_field( $settings, $value, $tag, $single = false ) {
	...
	$output .= '<div class="gallery_widget_attached_images is-empty">';
	...
	$output .= '</div>';
	if ( true === $single ) {
		$output .= '<a class="gallery_widget_add_images" href="javascript:;" use-single="true" title="' . esc_attr__( 'Add image', 'js_composer' ) . '"><i class="la la-plus"></i>' . esc_html__( 'Add image', 'js_composer' ) . '</a>';
	} else {
		$output .= '<a class="gallery_widget_add_images" href="javascript:;" title="' . esc_attr__( 'Add images', 'js_composer' ) . '"><i class="la la-plus"></i>' . esc_html__( 'Add images', 'js_composer' ) . '</a>';
	}
