...
if ( ! class_exists( 'WPBakeryVisualComposerCssEditor' ) ) {
	...
	class WPBakeryVisualComposerCssEditor {
		...
		public function render() {
			...
			$output .= sprintf( '<div class="vc_col-xs-5 vc_settings"><label>%s</label><div class="color-group"><input type="text" name="border_color" value="" class="vc_color-control"></div><label>%s</label><div class="vc_border-style"><select name="border_style" class="vc_border-style">%s</select></div><label>%s</label><div class="vc_border-radius"><select name="border_radius" class="vc_border-radius">%s</select></div><label>%s</label><div class="color-group"><input type="text" name="background_color" value="" class="vc_color-control"></div><div class="vc_background-image">%s</div><div class="vc_background-style"><select name="background_style" class="vc_background-style">%s</select></div><label>%s</label><label class="vc_checkbox-nolabel"><input type="checkbox" name="simply" class="vc_simplify" value=""> <span>%s</span></label></div>', esc_html__( 'Border color', 'js_composer' ), esc_html__( 'Border style', 'js_composer' ), $this->getBorderStyleOptions(), esc_html__( 'Border radius', 'js_composer' ), $this->getBorderRadiusOptions(), esc_html__( 'Background', 'js_composer' ), $this->getBackgroundImageControl(), $this->getBackgroundStyleOptions(), esc_html__( 'Box controls', 'js_composer' ), esc_html__( 'Simplify controls', 'js_composer' ) );
		...
		public function getBackgroundImageControl() {
			$value = sprintf( '<ul class="vc_image"></ul><a href="#" class="vc_add-image"><i class="la la-plus"></i>%s</a>', esc_html__( 'Add image', 'js_composer' ) );
		...
		public function getBackgroundStyleOptions() {
			...
			$styles = apply_filters( 'vc_css_editor_background_style_options_data', array(
				...
				esc_html__( 'Repeat X', 'js_composer' ) => 'repeat-x',
				esc_html__( 'Repeat Y', 'js_composer' ) => 'repeat-y',
			) );
