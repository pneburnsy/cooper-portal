...
class FormidableForms {
	...
	public function field_options_form( $field, $display, $values ) {
        ...
        <tr>
            <td><h2>Configure Uploader</h2></td>
            <td>
	...
	public function frontend_field( $field, $field_name, $atts ) {
		...
		echo sprintf( "<input type='hidden' name='%s' id='%s' class='upload-file-list' value='%s'>", $field_name, $atts['html_id'], $prefill );
	}
