	...
	<?php if ( $is_dokan_upload ) { ?>
		...
		<form class="dokan-form-horizontal" method="post" id="igd-dokan-upload-settings">
			...
			<div class="dokan-form-group">
				<label class="dokan-w3 dokan-control-label dokan-text-left" for="igd_upload_folder">
					<?php
					$can_delete = get_user_meta( dokan_get_current_user_id(), '_igd_dokan_can_delete', true );
					esc_html_e( 'Can Delete Uploaded Files', 'integrate-google-drive' ); ?> </label>

				<div class="dokan-w9 dokan-text-left">
					<label>
						<input type="checkbox" name="igd_dokan_can_delete"
						       value="yes" <?php checked( $can_delete, 'yes' ); ?> />
						<?php esc_html_e( 'Allow customers to delete uploaded files', 'integrate-google-drive' ); ?>
					</label>
				</div>
			</div>
