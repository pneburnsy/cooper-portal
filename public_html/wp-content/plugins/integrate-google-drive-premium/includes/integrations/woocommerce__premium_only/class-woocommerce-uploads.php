...
class WooCommerce_Uploads {
	...
	public function __construct() {
		...
		// Remove files
		add_action( 'wp_ajax_igd_wc_remove_file', [ $this, 'remove_file' ] );
		add_action( 'wp_ajax_nopriv_igd_wc_remove_file', [ $this, 'remove_file' ] );
	...
	public function remove_file() {
		...
		//Remove uploaded files fromwc session
	...
	public function render_uploader( $product, $order = null ) {
		...
		<div class="igd-wc-upload-wrap">
			...
			$this->render_file_list( $files, $product_id );
	...
	public function render_file_list( $files, $product_id = null ) { ?>
		<div class="wc-file-list-wrap <?php echo esc_attr( empty( $files ) ? 'hidden' : '' ); ?>">
			...
			<ul class="igd-file-list wc-file-list">
				<?php

				if ( $this->is_dokan_active() ) {
					$can_delete = 'yes' == get_user_meta( get_post_field( 'post_author', $product_id ), '_igd_dokan_can_delete', true );
				} else {
					$can_delete = igd_get_settings( 'wooCommerceCanDeleteUploadedFiles', false );
				}
						...
						<li>
							...
							<?php if ( $can_delete ) { ?>
								<i
									...
									data-account-id="<?php echo esc_attr( $file['accountId'] ); ?>"
									title="<?php esc_attr_e( 'Remove', 'integrate-google-drive' ); ?>"
									aria-label="<?php esc_attr_e( 'Remove', 'integrate-google-drive' ); ?>"
									class="remove-file dashicons dashicons-no-alt"></i>
							<?php } ?>
			...
			<div class="options_group">
				...
				<div class="show_if_igd_upload upload-box-settings">
					...
					<p class="form-field upload-file-extensions-field">
						<label
							for="upload_file_extensions"><?php _e( 'Allowed File Types', 'integrate-google-drive' ); ?></label>
