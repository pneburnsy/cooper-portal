...
(function ($) {
  ...
  class Plugin {
    ...
    getStickyElements() {
      ...
      }) : this.DOM.$element.find('.lqd-head-sec-wrap, .lqd-stickybar-wrap').find('.lqd-head-col > .header-module');
...
(function ($) {
  ...
  class Plugin {
    ...
    navigate(dir = 'next', navigatedByUser) {
      ...
      currentSlideLink?.length && currentSlideContentTimeline.fromTo(currentSlideLink, {
        opacity: 1,
        y: '0%'
      }, {
        opacity: 0,
      ...
      newSlideLink?.length && newSlideContent.fromTo(newSlideLink, {
        opacity: 0,
        y: dir === 'next' ? '75%' : '-75%'
      }, {
        opacity: 1,
  ...
  Plugin.prototype = {
    ...
    addSlidesCurrentNumbers() {
      ...
      for (let i = 1; i <= flickityData.slides.length; i++) {
        $currentInner.append(`<span class="d-inline-block" style="width: auto; text-indent: 0;">${i < 10 && isZeroBased ? '0' : ''}${i}</span>`);
      }
      setTimeout(() => {
        const widths = $currentInner.children('span').map((i, el) => $(el).outerWidth(true));
        this.$carouselCurrentSlide.css('width', Math.ceil(Math.max(...widths)));
      }, 0);
    ...
    changeSlidesNumbers() {
      ...
      this.$carouselCurrentSlide.css('width', $(selectedNum).outerWidth(true));
...
(function ($) {
  ...
  class Plugin {
    ...
    onShown(event) {
      ...
      if (!liquidIsMobile() && offsetTop <= $liquidWindow.scrollTop() - 15 || liquidIsMobile() && offsetTop > $liquidWindow.scrollTop() + window.innerHeight) {
...
jQuery(document).ready(function ($) {
  if (typeof $?.fn?.tab === 'undefined') return;
  $('.lqd-tabs').liquidTab();
});
(function ($) {
  'use strict';
...
(function ($) {
  ...
  class Plugin {
    ...
    checkIsInView(element, ScrollPos = this.getScrollPos()) {
    ...
    getVisibilityPercent(targetRect, ScrollPos = this.getScrollPos()) {
      if (!this.checkIsInView(targetRect.el, ScrollPos)) return 0;
  ...
  class Plugin {
    ...
    async _runAnimations() {
      ...
      this.animationTargets.forEach((target, i) => {
        if (!target.vars) {
          this.animationsTimeline.to(target, {
            ...animations,
            onStart: () => {
